...
def unfolding_covariance(
    reco_counts: np.ndarray,
    response: np.ndarray,
    prior: np.ndarray,
    iterations: int,
) -> tuple[np.ndarray, np.ndarray]:
    unfolded = iterative_bayes(reco_counts, response, prior, iterations)
    jacobian = np.empty((prior.size, reco_counts.size), dtype=float)
    for index in range(reco_counts.size):
        step = max(1.0, 0.01 * np.sqrt(max(reco_counts[index], 1.0)))
        upper = reco_counts.copy()
        upper[index] += step
        if reco_counts[index] >= step:
            lower = reco_counts.copy()
            lower[index] -= step
            jacobian[:, index] = (
                iterative_bayes(upper, response, prior, iterations)
                - iterative_bayes(lower, response, prior, iterations)
            ) / (2.0 * step)
        else:
            jacobian[:, index] = (iterative_bayes(upper, response, prior, iterations) - unfolded) / step
    covariance = jacobian @ np.diag(np.clip(reco_counts, 1.0, None)) @ jacobian.T
    return unfolded, 0.5 * (covariance + covariance.T)
...
def reweighted_truth_templates(
    ...
    bin_index = np.searchsorted(truth_edges, nominal_truth, side="right") - 1
    bin_index = np.clip(bin_index, 0, truth_edges.size - 2)
    templates = []
    denominator = 1.0 + nominal_C * nominal_truth
    for C in scan:
        weights = (1.0 + C * nominal_truth) / denominator
        template = np.bincount(bin_index, weights=weights, minlength=truth_edges.size - 1).astype(float)
...
def fit_parameter(
    ...
    scan: np.ndarray,
    covariance_rcond: float = 1.0e-8,
) -> tuple[float, float, float, np.ndarray]:
    inverse = np.linalg.pinv(covariance, rcond=covariance_rcond, hermitian=True)
    residual = unfolded[None, :] - templates
    chi2 = np.einsum("si,ij,sj->s", residual, inverse, residual)
    best_index = int(np.argmin(chi2))
    best = float(scan[best_index])
    minimum = float(chi2[best_index])
    local_sigma: float | None = None
    if 0 < best_index < scan.size - 1:
        coefficients = np.polyfit(scan[best_index - 1 : best_index + 2], chi2[best_index - 1 : best_index + 2], 2)
